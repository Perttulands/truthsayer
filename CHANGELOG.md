...
### Added
- Judgment system design captured in `JUDGMENT.md`, with the supported implementation living in the Go `truthsayer judge` command
...
- `br` (beads_rust) replaces `bd` (beads) as the agent workflow CLI throughout codebase
...
### Removed
- Legacy shell prototype `scripts/judge.sh`; `truthsayer judge` is the only supported judgment surface
